...
local function IsSortConstraints(ply)
	return tobool(ply:GetInfo("advdupe2_sort_constraints"))
end

local function IsCopyOnlyMine(ply)
	return tobool(ply:GetInfo("advdupe2_sort_constraints"))
end

local function IsOffsetWorld(ply)
	return tobool(ply:GetInfo("advdupe2_offset_world"))
end
-- IsOriginalOrigin(ply)
local function IsOriginalOrigin(ply)
	return tobool(ply:GetInfo("advdupe2_original_origin"))
end

local function GetAreaCopySize(ply)
	return math.Clamp(tonumber(ply:GetInfo("advdupe2_area_copy_size")) or 50, 0, 30720)
end

local function IsAutoSaveContraption(ply)
	return tobool(ply:GetInfo("advdupe2_auto_save_contraption"))
end

local function GetAutoSaveTime(ply)
	return math.Clamp(tonumber(ply:GetInfo("advdupe2_auto_save_time")) or 2, 2, 30)
end

local function GetContrSpawnerDelay(ply)
	return tonumber(ply:GetInfo("advdupe2_contr_spawner_delay"))
end
...
if (SERVER) then
	...
	local function GroupConstraintOrder(ply, constraints)
		...
		local fullSystems = {}
		local function buildSystems(intab)
			while next(intab) ~= nil do
				for k, v in pairs(intab) do
					for sysi, sysv in pairs(sortingSystems) do
		...
		if #fullSystems ~= 0 then
			ply:ChatPrint("DUPLICATOR: WARNING, Number of constraints exceeds 100: (" .. #ret ..
											"). Constraint sorting might not work as expected.")
		end
	...
	local areacopy_classblacklist = {gmod_anchor = true}

	local function PlayerCanDupeCPPI(ply, ent)
		if not AdvDupe2.duplicator.IsCopyable(ent) or areacopy_classblacklist[ent:GetClass()] then
			return false
		end
	...
	function TOOL:LeftClick(trace)
		...
		if (not ply.AdvDupe2 or not ply.AdvDupe2.Entities) then
			return false
		end
		...
		if IsOffsetWorld(ply) then
			local ent = ply.AdvDupe2.Entities[ply.AdvDupe2.HeadEnt.Index]
			ply.AdvDupe2.Angle = ply.AdvDupe2.Angle - ent.PhysicsObjects[0].Angle
		end

		ply.AdvDupe2.Pasting = true
		AdvDupe2.Notify(ply, "Pasting...")
		local origin
		if IsOriginalOrigin(ply) then
	...
	function TOOL:RightClick(trace)
		...
		if (self:GetStage() == 1) then
			...
			Entities, Constraints = AdvDupe2.duplicator.AreaCopy(Ents, HeadEnt.Pos, IsCopyOutside(ply))
		...
		elseif trace.HitNonWorld then -- Area Copy is off
			...
			if ply.AdvDupe2.Entities then
				...
				Entities, Constraints = AdvDupe2.duplicator.AreaCopy(Entities, HeadEnt.Pos, IsCopyOutside(ply))
			end
		end
	...
	function TOOL:Deploy()
		...
		if not ply.AdvDupe2.Entities then
			return
		end
	...
	function TOOL:Reload(trace)
		...
		if (self:GetStage() == 1) then
			...
			ply.AdvDupe2.TempAutoSaveOutSide = IsCopyOutside(ply)
			return true
		end
		...
		if (trace.Entity:GetClass() == "gmod_contr_spawner") then
			local delay = GetContrSpawnerDelay(ply)
	...
	function MakeContraptionSpawner(ply, Pos, Ang, HeadEnt, EntityTable, ConstraintTable, delay, undo_delay, model, key,
																	undo_key, disgrav, disdrag, addvel, hideprops)

		if not ply:CheckLimit("gmod_contr_spawners") then
			return nil
		end
	...
	net.Receive("AdvDupe2_CanAutoSave", function(len, ply, len2)
		...
		if (ent ~= 0) then
			...
			if IsAutoSaveContraption(ply) then
		...
		ply.AdvDupe2.AutoSaveContr   = IsAutoSaveContraption(ply)
		ply.AdvDupe2.AutoSaveDesc    = desc

		local time = GetAutoSaveTime(ply)
		...
		timer.Create(name, time * 60, 0, function()
			...
			if (ply.AdvDupe2.AutoSaveContr) then
				...
				Tab.Entities, Tab.Constraints = AdvDupe2.duplicator.AreaCopy(Entities, Tab.HeadEnt.Pos,
																																		 ply.AdvDupe2.AutoSaveOutSide)
			end
...
if (CLIENT) then
	...
	local function MouseWheelScrolled(ply, bind, pressed)
		...
		if (bind == "invprev") then
			if (ply:GetTool("advdupe2"):GetStage() == 1) then
				local size = math.min(GetAreaCopySize(ply) + 25, 30720)
				RunConsoleCommand("advdupe2_area_copy_size", size)
			else
				RunConsoleCommand("advdupe2_offset_z", GetDupeElevation(ply) + 5)
			end
			return true
		elseif (bind == "invnext") then
			if (ply:GetTool("advdupe2"):GetStage() == 1) then
				local size = math.max(GetAreaCopySize(ply) - 25, 25)
				RunConsoleCommand("advdupe2_area_copy_size", size)
			else
				RunConsoleCommand("advdupe2_offset_z", GetDupeElevation(ply) - 5)
			end
	...
	language.Add("Tool.advdupe2.0",
							 "Primary: Paste, Secondary: Copy, Secondary+World: Select/Deselect All, Secondary+Shift: Area copy.")
	language.Add("Tool.advdupe2.1",
							 "Primary: Paste, Secondary: Copy an area, Reload: Autosave an area, Secondary+Shift: Cancel.")
	...
	local function BuildCPanel(CPanel)
		...
		end -- Fix the damned mouse not scrolling when it's over the catagories
		...
		CategoryContent3.OnMouseWheeled = function(self, dlta)
			parent:OnMouseWheeled(dlta)
		end
		...
		CategoryContent4.OnMouseWheeled = function(self, dlta)
			parent:OnMouseWheeled(dlta)
		end
		...
		btn.DoClick = function()
			...
			FileBrowser.Submit.DoClick = function()
				...
				if (not IsValid(FileBrowser.Browser.pnlCanvas.m_pSelectedItem) or
					FileBrowser.Browser.pnlCanvas.m_pSelectedItem.Derma.ClassName ~= "advdupe2_browser_folder") then
		...
		btn.DoClick = function()
			if (AdvDupe2.AutoSavePos) then
				...
				ply:SetEyeAngles((AdvDupe2.AutoSavePos - ply:GetShootPos()):Angle())
				RunConsoleCommand("AdvDupe2_SetStage")
				hook.Add("HUDPaint", "AdvDupe2_DrawSelectionBox", AdvDupe2.DrawSelectionBox)
			end
		end
		...
		CategoryContent5.OnMouseWheeled = function(self, dlta)
			parent:OnMouseWheeled(dlta)
		end
		...
		if (ply:IsAdmin()) then
			...
			CategoryContent6.OnMouseWheeled = function(self, dlta)
				parent:OnMouseWheeled(dlta)
			end
	...
	function TOOL.BuildCPanel(panel)
		panel:ClearControls()
		panel:AddControl("Header", {Text = "Advanced Duplicator 2", Description = "Duplicate stuff."})
	...
	function TOOL:DrawToolScreen()
		...
		rate, ply = (FrameTime() * 160), LocalPlayer()
		...
		draw.SimpleText("Advanced Duplicator 2", "AD2TitleFont", 128, 50, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER,
										TEXT_ALIGN_TOP)
	...
	function AdvDupe2.DrawSelectionBox()
		...
		if (not AdvDupe2.LastUpdate or CurTime() >= AdvDupe2.LastUpdate) then
			...
			local Entities = FindInBox(B1, (Vector(i, i, i) + TraceRes.HitPos), ply)
