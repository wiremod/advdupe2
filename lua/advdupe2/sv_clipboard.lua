  ...
  local function recursiveCopy(oldtable)
    ...
    for k, v in pairs(oldtable) do
      ...
      if serializable[varType] then
        ...
        print(
          "[AdvDupe2] ClassArg table with key \"" .. tostring(k) .. "\" has unsupported value of type \"" .. type(v) ..
            "\"!\n")
      end
    end
    return newtable
...
local function CopyEntTable(Ent, Offset)
  ...
  if EntityClass then
    for iNumber, Key in pairs(EntityClass.Args) do
      ...
      if Key ~= "Pos" and Key ~= "Model" and Key ~= "Ang" and Key ~= "Angle" and Key ~= "ang" and Key ~= "angle" and Key ~=
        "pos" and Key ~= "position" and Key ~= "model" then
  ...
  if (Ent.EntityMods) then Tab.EntityMods = table.Copy(Ent.EntityMods) end

  if Ent.PostEntityCopy then
    local status, valid = pcall(Ent.PostEntityCopy, Ent)
    if (not status) then print("AD2 PostEntityCopy Error: " .. tostring(valid)) end
  end
  ...
  if (Tab.Class == "gmod_cameraprop") then Tab.key = Ent:GetNetworkedInt("key") end
  ...
  for k, v in pairs(Ent:GetBodyGroups()) do
    if (Ent:GetBodygroup(v.id) > 0) then Tab.BodyG[v.id] = Ent:GetBodygroup(v.id) end
  end

  if (table.Count(Tab.BodyG) == 0) then Tab.BodyG = nil end
  ...
  if Ent.OnEntityCopyTableFinish then
    ...
    if (not status) then print("AD2 OnEntityCopyTableFinish Error: " .. tostring(valid)) end
  end
...
local function Copy(Ent, EntTable, ConstraintTable, Offset)
      ...
      if Constraint:IsValid() then
        ...
        if index and not ConstraintTable[index] then
          ...
          for j, e in pairs(EntTab) do
            if e and (e:IsWorld() or e:IsValid()) then Copy(e, EntTable, ConstraintTable, Offset) end
          end
        end
      end
    end
  ...
  do -- Wiremod Wire Connections
    if istable(Ent.Inputs) then
      for k, v in pairs(Ent.Inputs) do
        if isentity(v.Src) and v.Src:IsValid() then Copy(v.Src, EntTable, ConstraintTable, Offset) end
      end
    end
  ...
  do -- Parented stuff
    ...
    for k, child in pairs(Ent:GetChildren()) do Copy(child, EntTable, ConstraintTable, Offset) end
  end

  for k, v in pairs(EntTable[Ent:EntIndex()].PhysicsObjects) do Ent:GetPhysicsObjectNum(k):EnableMotion(v.Frozen) end
...
function AdvDupe2.duplicator.AreaCopy(Entities, Offset, CopyOutside)
  ...
  for _, Constraint in pairs(Constraints) do
    ...
    if (not CopyOutside) then
      add = true
      for k, v in pairs(EntTab) do if (not Entities[v:EntIndex()]) then add = false end end
      ...
      for k, v in pairs(EntTab) do Copy(v, EntTable, ConstraintTable, Offset) end
    end
  end
...
local function CreateConstraintFromTable(Constraint, EntityList, EntityTable, Player, DontEnable)
  ...
  if(Constraint.Type:lower() == "slider") then
    print("CreateConstraint: "..Constraint.Type)
  end
  ...
  for k, Key in pairs(Factory.Args) do
        ...
        if Key == "Ent" .. i or Key == "Ent" then
          if (Constraint.Entity[i].World) then
            ...
            if not IsValid(Val) then
              ...
              if (IsValid(Val:GetPhysicsObject())) then Val:GetPhysicsObject():EnableMotion(false) end
    ...
    if(Constraint.Type:lower() == "slider") then
      print("["..tostring(k).."]<"..tostring(Key).."> = ["..type(Val).."]<"..tostring(Val)..">{"..tostring(Par).."}")
      if(type(Val) == "Vector") then
        print("LEN: "..tostring(Val:LengthSqr()))
      end
    end
  ...
  if(Constraint.Type:lower() == "slider") then
    print("Create slider constraint: "..tostring(Ent))
  end
  ...
  if (EntityTable) then
    if (first ~= nil and not first:IsWorld()) then
      ...
      if (IsValid(Bone1) and Bone1Index ~= 0) then
        Bone1:SetPos(EntityTable[firstindex].BuildDupeInfo.PosReset +
                       EntityTable[firstindex].BuildDupeInfo.PhysicsObjects[Bone1Index].Pos)
        Bone1:SetAngles(EntityTable[firstindex].PhysicsObjects[Bone1Index].Angle)
      end

      local FirstPhys = first:GetPhysicsObject()
      if IsValid(FirstPhys) then
        if ReEnableFirst then FirstPhys:EnableMotion(true) end
      end
    end
    ...
    if (second ~= nil and not second:IsWorld()) then
      ...
      if (IsValid(Bone2) and Bone2Index ~= 0) then
        Bone2:SetPos(EntityTable[secondindex].BuildDupeInfo.PosReset +
                       EntityTable[secondindex].BuildDupeInfo.PhysicsObjects[Bone2Index].Pos)
        Bone2:SetAngles(EntityTable[secondindex].PhysicsObjects[Bone2Index].Angle)
      end

      local SecondPhys = second:GetPhysicsObject()
      if IsValid(SecondPhys) then
        if ReEnableSecond then SecondPhys:EnableMotion(true) end
      end
    end
  end
...
local function MakeProp(Player, Pos, Ang, Model, PhysicsObject, Data)
  ...
  if (Player) then if (not gamemode.Call("PlayerSpawnProp", Player, Model)) then return false end end
...
local function RestoreBodyGroups(ent, BodyG) for k, v in pairs(BodyG) do ent:SetBodygroup(k, v) end end
...
local function CreateEntityFromTable(EntTable, Player)
  ...
  if (not GENERIC) then
    ...
    elseif IsAllowed(Player, EntTable.Class, EntityClass) then
      ...
      if (not EntTable.BuildDupeInfo.IsVehicle and not EntTable.BuildDupeInfo.IsNPC and EntTable.Class ~= "prop_ragdoll" and
        EntTable.Class ~= "prop_effect") then -- These four are auto done
  ...
  if (status ~= false and IsValid(valid)) then
    if (sent) then
      ...
      if valid.RestoreNetworkVars then valid:RestoreNetworkVars(EntTable.DT) end
...
function AdvDupe2.duplicator.Paste(Player, EntityList, ConstraintList, Position, AngleOffset, OrigPos, Parenting)
  ...
  for k, Constraint in pairs(ConstraintList) do
    ...
    if (IsValid(Entity)) then table.insert(CreatedConstraints, Entity) end
  end
  ...
  if (Player) then
    ...
    for _, v in pairs(CreatedEntities) do
      ...
      if v.PostEntityPaste then
        ...
        if (not status) then print("AD2 PostEntityPaste Error: " .. tostring(valid)) end
      end
...
local function AdvDupe2_Spawn()
  ...
  if (not Queue or not IsValid(Queue.Player)) then
    if Queue then table.remove(AdvDupe2.JobManager.Queue, AdvDupe2.JobManager.CurrentPlayer) end
  ...
  if (Queue.Entity) then
    ...
    if (not v.BuildDupeInfo) then v.BuildDupeInfo = {} end
    if (v.LocalPos) then
      for i, p in pairs(v.PhysicsObjects) do v.PhysicsObjects[i] = {Pos = v.LocalPos, Angle = v.LocalAngle} end
    end
    ...
    if (#Queue.ConstraintList > 0) then
      ...
      if IsValid(Entity) then table.insert(Queue.CreatedConstraints, Entity) end
    elseif (table.Count(Queue.ConstraintList) > 0) then
      local tbl = {}
      for k, v in pairs(Queue.ConstraintList) do table.insert(tbl, v) end
    ...
    if (Queue.Current > #Queue.ConstraintList) then
      ...
      for _, v in pairs(Queue.CreatedEntities) do
        if (not IsValid(v)) then
          ...
          if (not edit or not Queue.DisableParents) then v:SetNotSolid(v.SolidMod) end
...
local function ErrorCatchSpawning()
  ...
  while ticktotal >= 1 do
    ...
    if (not status) then
      ...
      for k, v in pairs(Queue.CreatedEntities) do if (IsValid(v)) then v:Remove() end end

      if (IsValid(Queue.Player)) then AdvDupe2.FinishPasting(Queue.Player, true) end
      ...
      if (#AdvDupe2.JobManager.Queue == 0) then
        ...
        if (#Queue < AdvDupe2.JobManager.CurrentPlayer) then AdvDupe2.JobManager.CurrentPlayer = 1 end
      end
...
local function RemoveSpawnedEntities(tbl, i)
  ...
  for k, v in pairs(AdvDupe2.JobManager.Queue[i].CreatedEntities) do if (IsValid(v)) then v:Remove() end end
...
function AdvDupe2.InitPastingQueue(Player, PositionOffset, AngleOffset, OrigPos, Constrs, Parenting, DisableParents,
  ...
  for k, v in pairs(Player.AdvDupe2.Entities) do table.insert(Queue.SortedEntities, k) end
