...
local function CollapseParentsComplete(node)
	if not node.ParentNode.ParentNode then
		node:SetExpanded(false) -- CollapseChildren(node) return end
		return
	end
...
function BROWSER:DoNodeRightClick(node)
		...
		if node.Control.Search then
			...
			Menu:AddOption("Delete", 	function()
												...
												parent.Submit.DoClick = function()
																			...
																			if area == 2 then -- cant you use else if here?
	...
	if not node.Control.Search then
		...
		Menu:AddOption("Collapse Root", function()
			CollapseParentsComplete(node)
		end)
		if parent.Expanded then
			Menu:AddOption("Cancel Action", function()
				parent.Cancel:DoClick()
			end)
		end
	end
...
function BROWSER:Sort(node)
	table.sort(node.Folders, function(a, b) return
		a.Label:GetText() < b.Label:GetText()
	end)
...
function FOLDER:SetExpanded(bool)
	...
	if not bool then
		self.m_bExpanded = not self.m_bExpanded else self.m_bExpanded = bool
		end
...
function PANEL:Init()
	...
	self.Help.DoClick = function(btn)
		...
		Menu:AddOption("Commands", function()
			gui.OpenURL("https://github.com/wiremod/advdupe2/wiki/Server-settings")
		end)
		Menu:Open()
	end
...
if game.SinglePlayer() then
	-- TODO: usermessage is deprecated use net
